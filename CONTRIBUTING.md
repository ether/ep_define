# Contributing to ep_define

Thanks for helping improve `ep_define` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_define` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_define from your local clone
pnpm run plugins i --path ../ep_define

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_define/` are picked up after a server restart.
...
* Etherpad version, ep_define version, browser + OS
* Steps to reproduce
