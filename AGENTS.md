# Agent Guide — ep_define

Define and Research Words.
...
`ep_define` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_define`:
...
* Server: `eejsBlock_mySettings`, `eejsBlock_dd_help`, `handleMessage`
